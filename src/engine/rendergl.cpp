...
void renderbreakoverlay(const ivec &origin, int size, int stage)
{
    ...
    const int column = stage % 4, row = stage / 4;
    const float inset = 0.05f,
                x0 = origin.x - inset, y0 = origin.y - inset, z0 = origin.z - inset,
                x1 = origin.x + size + inset, y1 = origin.y + size + inset,
                z1 = origin.z + size + inset,
                u0 = (column * 16.0f + 0.5f) / 64.0f,
                u1 = (column * 16.0f + 15.5f) / 64.0f,
                v0 = (row * 16.0f + 0.5f) / 32.0f,
                v1 = (row * 16.0f + 15.5f) / 32.0f;

    ldrshader->set();
    glBindTexture(GL_TEXTURE_2D, destroytex->id);
    ...
    gle::defvertex();
    gle::deftexcoord0();
    gle::begin(GL_QUADS, 24);

    #define BREAKVERT(x, y, z, u, v) \
        gle::attribf(x, y, z); \
        gle::attribf(u, v)

    BREAKVERT(x0, y0, z0, u0, v1);
    BREAKVERT(x0, y1, z0, u1, v1);
    BREAKVERT(x0, y1, z1, u1, v0);
    BREAKVERT(x0, y0, z1, u0, v0);

    BREAKVERT(x1, y1, z0, u0, v1);
    BREAKVERT(x1, y0, z0, u1, v1);
    BREAKVERT(x1, y0, z1, u1, v0);
    BREAKVERT(x1, y1, z1, u0, v0);

    BREAKVERT(x1, y0, z0, u0, v1);
    BREAKVERT(x0, y0, z0, u1, v1);
    BREAKVERT(x0, y0, z1, u1, v0);
    BREAKVERT(x1, y0, z1, u0, v0);

    BREAKVERT(x0, y1, z0, u0, v1);
    BREAKVERT(x1, y1, z0, u1, v1);
    BREAKVERT(x1, y1, z1, u1, v0);
    BREAKVERT(x0, y1, z1, u0, v0);

    BREAKVERT(x0, y0, z0, u0, v1);
    BREAKVERT(x1, y0, z0, u1, v1);
    BREAKVERT(x1, y1, z0, u1, v0);
    BREAKVERT(x0, y1, z0, u0, v0);

    BREAKVERT(x0, y0, z1, u0, v1);
    BREAKVERT(x0, y1, z1, u1, v1);
    BREAKVERT(x1, y1, z1, u1, v0);
    BREAKVERT(x1, y0, z1, u0, v0);
